...
export default function Newsfeed({ navigation }) {
    ...
    const handlePost = async () => {
        // Check if both postTitle and postText are defined and not empty
        if (!postTitle || !postText || postTitle.trim() === '' || postText.trim() === '') {
            ...
            <ScrollView contentContainerStyle={{ flexGrow: 1 }} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
                <SafeAreaView style={styles.container}>
                    ...
                    <View>
                        ...
                        {BodyContent()}
                        {/* Display events when the "Events" button is clicked */}
                        {events.length > 0 && (
                            <View>
                                <Text style={{ fontSize: 20, fontWeight: 900, color: 'rgb(81,175,91)' }}>EVENTS</Text>
                                {EventsContent()}
                            </View>
                        )}
                    </View>
